        ...
        return $form
            ->schema([
                ...
                Section::make('Personal Information')
                    ...
                    ->columns(3)
                    ->schema([
                        // Matri ID — editable ONLY on create (optional; blank =
                        // auto-generated by the Profile model). On edit it stays a
                        // read-only display, since the ID is set once (used site-wide
                        // as the profile's identifier). Trimmed + uniqueness-checked.
                        Forms\Components\TextInput::make('matri_id')->label('Matri ID')
                            ->maxLength(10)
                            ->disabled(fn (string $operation): bool => $operation === 'edit')
                            ->dehydrated(fn (string $operation): bool => $operation === 'create')
                            ->dehydrateStateUsing(fn (?string $state): ?string => filled($state) ? trim($state) : null)
                            ->unique(table: 'profiles', column: 'matri_id', ignoreRecord: true)
                            ->helperText(fn (string $operation): ?string => $operation === 'create'
                                ? 'Optional — leave blank to auto-generate. Max 10 characters; must be unique. Use this to keep an existing (offline) ID.'
                                : null),
