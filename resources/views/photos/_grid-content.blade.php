{{--
    Photo management grid — shared partial used by both:
      • /manage-photos (resources/views/photos/manage.blade.php)
      • /onboarding/photo (resources/views/onboarding/photo.blade.php)

    Each parent view is responsible for its own surrounding chrome
    (breadcrumb header for manage; 5-step progress bar + onboarding heading
    + Done/Continue CTA for onboarding). The grid itself — profile slot,
    album/family tabs, archive accordion, and Cropper.js modal — is identical.

    Required variables (passed via @include):
      $profilePhoto, $albumPhotos, $familyPhotos, $archivedPhotos, $privacy
      $redirectTo (string) — full URL the upload endpoint should redirect to
                              after a successful save. Must match the page
                              hosting this partial; whitelisted server-side
                              against route('photos.manage') | route('onboarding.photo').
--}}
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="bg-white rounded-lg border border-gray-200 shadow-xs overflow-hidden">
                ...
                <div class="p-5">
                    {{-- Format info --}}
                    <p class="text-xs text-gray-400 mb-4">PNG/GIF/JPG/JPEG/WebP format with maximum {{ (int) config('matrimony.max_photo_size_mb', 5) }} MB size allowed.</p>
            ...
            <div x-show="!sourceImage" class="p-8">
                <label class="block w-full cursor-pointer">
                    <div class="border-2 border-dashed border-gray-300 rounded-xl p-12 text-center hover:border-(--color-primary) hover:bg-(--color-primary-light) transition-colors">
                        ...
                        <p class="text-xs text-gray-500">JPG, PNG, GIF, WebP. Max {{ (int) config('matrimony.max_photo_size_mb', 5) }} MB. You can crop and rotate after selecting.</p>
                    </div>
...
{{-- Alpine photo manager component (handles Cropper.js lifecycle + upload).
     ...
     `redirectTo` is supplied per-host so /onboarding/photo and /manage-photos
     each round-trip back to their own URL after a successful upload (the
     server validates it against a whitelist; see PhotoController::upload). --}}
<script>
    function photoManagerEditor(initialShowArchived, initialActiveTab, redirectTo) {
        return {
            ...
            loadIntoCropper(event) {
                ...
                if (file.size > {{ (int) config('matrimony.max_photo_size_mb', 5) }} * 1024 * 1024) {
            ...
            submitCropped() {
                ...
                canvas.toBlob((blob) => {
                    ...
                    form.append('redirect_to', this.redirectTo);
                    ...
                    .then((res) => {
                        this.submitting = false;
                        // Server decides destination via redirect_to (whitelisted).
                        // fetch follows the 302 automatically; res.url is final URL.
