...
<body class="bg-gray-50 text-[#1C1917] font-sans antialiased min-h-screen flex flex-col">
        ...
        <div class="max-w-5xl w-full flex flex-col lg:flex-row items-center gap-8 lg:gap-0">
            ...
            <div class="w-full lg:w-7/12">
                <div class="bg-white rounded-2xl shadow-lg p-8 sm:p-10">
                    <h2 class="text-xl sm:text-2xl font-serif font-bold text-(--color-primary) mb-6">Verify Your Email ID</h2>

                    @if (session('email_otp_sent') || $errors->has('otp'))
                        <p class="text-sm text-gray-600 mb-2 leading-relaxed">
                            Please enter the OTP which you have received on your email
                            <strong class="text-gray-900">{{ $email }}</strong>.
                        </p>
                        <p class="text-sm text-gray-500 mb-8">
                            Not your mail? Please update it now.
                            <a href="{{ route('register') }}" class="text-(--color-primary) hover:underline">&#9998;</a>
                        </p>
                    @else
                        <p class="text-sm text-gray-600 mb-8 leading-relaxed">
                            We will send a 6-digit OTP to <strong class="text-gray-900">{{ $email }}</strong> for verification.
                        </p>
                    @endif
                    ...
                    @else
                        <form method="POST" action="{{ route('register.verifyemailotp') }}" id="emailOtpForm">
                            @csrf

                            <div class="flex justify-center gap-3 sm:gap-4 mb-8">
                                @for($i = 0; $i < 6; $i++)
                                    <input type="text" maxlength="1" inputmode="numeric" placeholder="*"
                                        class="email-otp-box w-12 h-14 sm:w-14 sm:h-16 border border-gray-300 rounded-xl text-center text-xl font-semibold outline-none transition-all placeholder:text-gray-300"
                                        {{ $i === 0 ? 'autofocus' : '' }}>
                                @endfor
                            </div>
                    ...
                    <div class="mt-8 pt-5 border-t border-gray-100 flex items-center justify-between">
                        <a href="{{ route('register.step5') }}" class="text-sm text-gray-400 hover:text-gray-600">&larr; Back to Step 5</a>
                        <a href="{{ route('register.verify') }}" class="text-sm text-(--color-primary) hover:underline font-medium">Skip for now &rarr;</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const hidden = document.getElementById('emailOtpHidden');
