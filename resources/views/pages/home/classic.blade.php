                ...
                <div class="hero-form bg-white rounded-xl shadow-xl p-5 sm:p-6 shrink-0">
                    ...
                    <form method="POST" action="{{ route('register.store1') }}" class="space-y-3" x-data="{
                        ...
                        <div class="float-field relative">
                            <input :type="showPw ? 'text' : 'password'" name="password" id="hero_password" required minlength="8" maxlength="64" placeholder=" "
    ...
    <section class="py-12 sm:py-16 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-8">
                <h2 class="text-2xl sm:text-3xl font-serif font-bold text-gray-900">Featured Profiles</h2>
                <p class="mt-2 text-sm text-gray-600">Hand-picked profiles from our community</p>
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                @foreach($featuredProfiles as $p)
                    <x-profile-card :profile="$p" />
                @endforeach
            </div>
            <div class="text-center mt-8">
                <a href="{{ route('search.quick') }}" class="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-semibold text-(--color-primary) border border-(--color-primary) rounded-lg hover:bg-(--color-primary-light) transition-colors">
                    View All Profiles
